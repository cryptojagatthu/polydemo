...
export default function MarketPage() {
  ...
  // ⚠️ Interpreted as CENTS in UI (1–99)
  ...
  const fetchOrders = async () => {
    ...
    const res = await fetch(`/api/limit-orders/by-market/${market.slug}`, {
  ...
  // Fetch orders when token & market ready (and auth finished loading)
  useEffect(() => {
    if (!isLoading && token && market) {
      fetchOrders();
    }
  ...
  const getHoldingForSide = (s: string) =>
    userHoldings.find((h) => h.outcome === s) || { shares: 0 };
  ...
  const placeMarketOrder = async () => {
    ...
    try {
      ...
      if (data.success) {
        setMessage(
          `✅ ${mode === 'buy' ? 'Bought' : 'Sold'} ${quantity} ${side} shares for $${total.toFixed(
            2
          )}`
        );
        try {
  ...
  // Limit order (POLYMARKET CENT SYSTEM)
  const placeLimitOrder = async () => {
    ...
    if (limitPrice === '' || isNaN(Number(limitPrice))) {
      setMessage('❌ Provide a valid price in cents (1–99)');
      return;
    }
    ...
    if (priceCents < 1 || priceCents > 99) {
      setMessage('❌ Price must be between 1 and 99 cents');
      return;
    }

    // ✅ Convert cents → decimal for backend (0.05, 0.63, etc.)
    ...
    try {
      const payload = {
        ...
        // ✅ Always send decimal to backend
        limitPrice: priceDecimal,
        expiresAt: computeExpiryIso(expiresOption),
      };
      ...
      if (data.success) {
        setMessage(`✅ Limit order placed — ${quantity} @ ${priceCents}¢`);
        await fetchHoldings();
        await fetchOrders();
        try {
          await refreshUser?.();
  ...
  // For UI cost preview when LIMIT mode: convert cents → decimal
  const limitPriceDecimalForUi =
    limitPrice === '' ? null : Number(limitPrice) / 100;
        ...
        <div className="max-w-4xl mx-auto">
              ...
              {market.outcomes.map((o) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 text-center border rounded cursor-pointer transition ${side === o.label
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-3 py-1 rounded font-semibold transition ${tradeMode === 'MARKET'
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded font-semibold transition ${tradeMode === 'LIMIT'
                      ? 'bg-gray-800 text-white'
                      : 'bg-gray-100 text-gray-700'
                    }`}
                >
            ...
            <div className="flex gap-2 mb-4">
              <button
                onClick={() => setMode('buy')}
                className={`px-4 py-2 rounded font-semibold w-full transition ${mode === 'buy'
              ...
              <button
                ...
                className={`px-4 py-2 rounded font-semibold w-full transition ${mode === 'sell'
            ...
            <div className="space-y-4">
                ...
                <>
                  ...
                  <div>
                    ...
                    <select
                      value={expiresOption}
                      onChange={(e) =>
                        setExpiresOption(e.target.value as any)
                      }
                      className="w-full border rounded px-4 py-2"
                    >
                ...
                <div className="flex justify-between mb-2">
                  ...
                  <span className="font-semibold">
                    {tradeMode === 'MARKET'
                      ...
                      : limitPriceDecimalForUi == null
                        ? '—'
                        : `$${limitPriceDecimalForUi.toFixed(3)}`}
                  </span>
                </div>
                <div className="flex justify-between text-lg font-bold">
                  ...
                  <span>
                    {tradeMode === 'MARKET'
                      ...
                      : limitPriceDecimalForUi == null
                        ? '—'
                        : `$${(limitPriceDecimalForUi * quantity).toFixed(2)}`}
                  </span>
                </div>
              </div>
              ...
              {mode === 'sell' && (userHoldingForSide.shares ?? 0) < quantity && (
                <div className="text-sm text-red-600">
                  You only have {userHoldingForSide.shares ?? 0} shares of{' '}
                  {side}. You cannot sell more than that.
                </div>
              )}
              ...
              <button
                ...
                className={`w-full py-3 rounded font-semibold transition ${placing || !user
              ...
              {message && (
                <div
                  className={`p-4 rounded ${message.includes('✅')
                ...
                {orders.map((order) => {
                  const lp =
                    order.limitPrice != null
                      ? Number(order.limitPrice) * 100
                      : order.fillPrice != null
                        ? Number(order.fillPrice) * 100
                        : null;
